...
<script>
import { nextTick, onActivated,onDeactivated, onBeforeMount, onBeforeUnmount, onBeforeUpdate, onErrorCaptured, onMounted, onRenderTracked, onRenderTriggered, onUnmounted, onUpdated } from 'vue';
export default {
  name:"LifeCycle",
  data(){
  ...
  setup(props, context){
    ...
    onBeforeUpdate(()=>{console.log('onBeforeUpdate执行')})

    // 因数据更新后页面重新渲染后执行 不能确保所有子组件都渲染完成 因此可以配合nextTick(()=>{})使用
    onUpdated(()=>{console.log('onUpdated执行')})

    // 跟踪虚拟 DOM 重新渲染时调用。钩子接收 debugger event 作为参数。此事件告诉你哪个操作跟踪了组件以及该操作的目标对象和键
    onRenderTracked((e)=>{console.log('onRenderTracked执行',e)})

    // 当虚拟 DOM 重新渲染为 triggered.Similarly 为renderTracked，接收 debugger event 作为参数。此事件告诉你是什么操作触发了重新渲染，以及该操作的目标对象和键
    onRenderTriggered((e)=>{console.log('onRenderTriggered执行',e)})

    // 在卸载组件实例之前调用。在这个阶段，实例仍然是完全正常的
    onBeforeUnmount(()=>{console.log('onBeforeUnmount执行')})
